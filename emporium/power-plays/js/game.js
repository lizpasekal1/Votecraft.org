...
class PowerPlaysGame {
    ...
    togglePause() {
        ...
        if (!this.isPaused) {
            ...
            if (player && !player.isHuman && this.state.phase !== GAME_PHASES.GAME_OVER) {
    ...
    async playCard(playerIndex, cardIndex, targetInfo = null) {
        ...
        await this.actionResolver.resolve(card, player, targetInfo);
    ...
    async advanceToNextTurn() {
        this.state.advancePlayer();

        // Break the async chain with setTimeout so previous turn contexts
        // can be garbage collected — prevents unbounded memory growth
    ...
    async endGame(powerWinner) {
        ...
        players.forEach(p => {
            if (p.lobbyCards.length > maxLobby) {
                maxLobby = p.lobbyCards.length;
                playsWinner = p;
            }
        });
