...
function exitConversation() {
    ...
    if (window.location.pathname.includes('character.html')) {
        // Navigate back to city page with city context
...
function loadCharacter(characterName = null) {
    ...
    const html = `
        ...
        <div class="cards-area">
            ...
            <div class="cards-title">Your response?</div>
        ...
        return `
            <div class="card-container">
                <div class="joke-card ${!canAfford ? 'disabled' : ''}">
                    ...
                    <div class="card-bookmark" data-bookmarked="false"><svg class="bookmark-outline" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#8B2635"><path d="M200-120v-640q0-33 23.5-56.5T280-840h400q33 0 56.5 23.5T760-760v640L480-240 200-120Zm80-122 200-86 200 86v-518H280v518Zm0-518h400-400Z"/></svg><svg class="bookmark-filled" style="display:none" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#8B2635"><path d="M200-120v-640q0-33 23.5-56.5T280-840h400q33 0 56.5 23.5T760-760v640L480-240 200-120Z"/></svg></div>
...
// Set up bookmark listeners (call after cards render)
function setupBookmarkListeners() {
    document.querySelectorAll('.card-bookmark').forEach(el => {
        function handleToggle(e) {
            e.preventDefault();
            e.stopPropagation();
            const isBookmarked = el.dataset.bookmarked === 'true';
            el.dataset.bookmarked = isBookmarked ? 'false' : 'true';
            el.querySelector('.bookmark-outline').style.display = isBookmarked ? '' : 'none';
            el.querySelector('.bookmark-filled').style.display = isBookmarked ? 'none' : '';
        }
        el.addEventListener('touchstart', handleToggle, { passive: false });
        el.addEventListener('click', function(e) {
            // Only fire on desktop (no touch)
            if (!('ontouchstart' in window)) handleToggle(e);
        });
    });
}
