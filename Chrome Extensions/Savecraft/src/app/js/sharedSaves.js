...
function buildVerticalCardSlider({ sectionClass, title, mobileTitle, cards, isListCard }) {
  ...
  const cardsHtml = tripled.map((c, i) => {
    ...
    // always gets the actual SaveCraft brand purple, not whichever color the rotation lands on;
    // every other avatar (real logo, photo, or emoji) keeps the normal rotating brand color.
    const color = c.name === 'Progressive' ? '#fff'
      : c.name === 'Votecraft' ? '#5B5BEF'
      : SHARED_VCARD_COLORS[i % SHARED_VCARD_COLORS.length];
  ...
  container.querySelectorAll('.shared-vcard--list').forEach(card => {
    card.addEventListener('click', () => {
      ...
      openSwitchConfirm({
        ...
        subtitle: 'The sidebar will display their specific saves. Explore their collection! Bookmark, star, or add a note to collect their saves into your saves library!',
