...
// Chapter/note/general "add a note" affordance — a plus rather than a star, since clicking it
// opens a note-taking field, not a favorite. Single icon (color toggles via the shared --active
// class) since there's no separate outline/filled variant for it. Song List keeps the pencil
// below instead — its rows are real track titles, not user-editable/renameable like these are.
...
// Song List's own per-track "add a note" affordance — unlike NOTE_OPEN_ICON above, still the
// original pencil (Song List has no rename control, so there's no second pencil in the same row
        ...
        tracklistEl.innerHTML = tracks.map(t => {
          ...
          <div class="detail-tracklist-item">
            <div class="detail-tracklist-row">
              ...
              <span class="detail-tracklist-favorite${isFav || hasNote ? ' detail-tracklist-favorite--active' : ''}" data-track-number="${t.number}">${SONG_NOTE_ICON}</span>
            </div>
