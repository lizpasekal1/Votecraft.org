// Auto-generated static lookup: curated Top 100 item title -> creator name.
// Unlike Music Album/Book, the live curated Movie/Show/Game Firestore data has NO creator
// embedded anywhere (plain titles, real description in .notes) — sourced externally via
// Wikidata (P57 director / P170 creator) and Steam appdetails, kept as static in-app data
// rather than rewriting 300 existing production curated_items documents' .title fields.

export const CURATED_MOVIE_DIRECTOR = {
  "Parasite": {
    "name": "Bong Joon-ho",
    "coDirectorCount": 1
  },
  "The Social Network": {
    "name": "David Fincher",
    "coDirectorCount": 1
  },
  "Superbad": {
    "name": "Greg Mottola",
    "coDirectorCount": 1
  },
  "Mad Max: Fury Road": {
    "name": "George Miller",
    "coDirectorCount": 1
  },
  "The Zone of Interest": {
    "name": "Jonathan Glazer",
    "coDirectorCount": 1
  },
  "Children of Men": {
    "name": "Alfonso Cuarón",
    "coDirectorCount": 1
  },
  "Inglourious Basterds": {
    "name": "Quentin Tarantino",
    "coDirectorCount": 1
  },
  "City of God": {
    "name": "Fernando Meirelles",
    "coDirectorCount": 2
  },
  "Crouching Tiger, Hidden Dragon": {
    "name": "Ang Lee",
    "coDirectorCount": 1
  },
  "Brokeback Mountain": {
    "name": "Ang Lee",
    "coDirectorCount": 1
  },
  "Y Tu Mamá También": {
    "name": "Alfonso Cuarón",
    "coDirectorCount": 1
  },
  "Zodiac": {
    "name": "David Fincher",
    "coDirectorCount": 1
  },
  "Mulholland Drive": {
    "name": "David Lynch",
    "coDirectorCount": 1
  },
  "The Wolf of Wall Street": {
    "name": "Martin Scorsese",
    "coDirectorCount": 1
  },
  "The Royal Tenenbaums": {
    "name": "Wes Anderson",
    "coDirectorCount": 1
  },
  "The Grand Budapest Hotel": {
    "name": "Wes Anderson",
    "coDirectorCount": 1
  },
  "Boyhood": {
    "name": "Richard Linklater",
    "coDirectorCount": 1
  },
  "Her": {
    "name": "Spike Jonze",
    "coDirectorCount": 1
  },
  "Phantom Thread": {
    "name": "Paul Thomas Anderson",
    "coDirectorCount": 1
  },
  "Anatomy of a Fall": {
    "name": "Justine Triet",
    "coDirectorCount": 1
  },
  "Adaptation": {
    "name": "Spike Jonze",
    "coDirectorCount": 1
  },
  "The Dark Knight": {
    "name": "Christopher Nolan",
    "coDirectorCount": 1
  },
  "Arrival": {
    "name": "Denis Villeneuve",
    "coDirectorCount": 1
  },
  "There Will Be Blood": {
    "name": "Paul Thomas Anderson",
    "coDirectorCount": 1
  },
  "Lost in Translation": {
    "name": "Sofia Coppola",
    "coDirectorCount": 1
  },
  "The Departed": {
    "name": "Martin Scorsese",
    "coDirectorCount": 1
  },
  "Bridesmaids": {
    "name": "Paul Feig",
    "coDirectorCount": 1
  },
  "A Separation": {
    "name": "Asghar Farhadi",
    "coDirectorCount": 1
  },
  "WALL-E": {
    "name": "Andrew Stanton",
    "coDirectorCount": 1
  },
  "A Prophet": {
    "name": "Jacques Audiard",
    "coDirectorCount": 1
  },
  "American Sniper": {
    "name": "Clint Eastwood",
    "coDirectorCount": 1
  },
  "Call Me by Your Name": {
    "name": "Luca Guadagnino",
    "coDirectorCount": 1
  },
  "Portrait of a Lady on Fire": {
    "name": "Céline Sciamma",
    "coDirectorCount": 1
  },
  "Lady Bird": {
    "name": "Greta Gerwig",
    "coDirectorCount": 1
  },
  "In the Mood for Love": {
    "name": "Wong Kar-wai",
    "coDirectorCount": 1
  },
  "Yi Yi": {
    "name": "Edward Yang",
    "coDirectorCount": 1
  },
  "Amélie": {
    "name": "Jean-Pierre Jeunet",
    "coDirectorCount": 1
  },
  "The Master": {
    "name": "Paul Thomas Anderson",
    "coDirectorCount": 1
  },
  "Oldboy": {
    "name": "Spike Lee",
    "coDirectorCount": 1
  },
  "Once Upon a Time in Hollywood": {
    "name": "Quentin Tarantino",
    "coDirectorCount": 1
  },
  "Moneyball": {
    "name": "Bennett Miller",
    "coDirectorCount": 1
  },
  "Roma": {
    "name": "Alfonso Cuarón",
    "coDirectorCount": 1
  },
  "Almost Famous": {
    "name": "Cameron Crowe",
    "coDirectorCount": 1
  },
  "The Lives of Others": {
    "name": "Florian Henckel von Donnersmarck",
    "coDirectorCount": 1
  },
  "Before Sunset": {
    "name": "Richard Linklater",
    "coDirectorCount": 1
  },
  "Moonlight": {
    "name": "Barry Jenkins",
    "coDirectorCount": 1
  },
  "Up": {
    "name": "Pete Docter",
    "coDirectorCount": 1
  },
  "12 Years a Slave": {
    "name": "Steve McQueen",
    "coDirectorCount": 1
  },
  "The Favourite": {
    "name": "Yorgos Lanthimos",
    "coDirectorCount": 1
  },
  "Borat": {
    "name": "Larry Charles",
    "coDirectorCount": 1
  },
  "Pan's Labyrinth": {
    "name": "Guillermo del Toro",
    "coDirectorCount": 1
  },
  "Inception": {
    "name": "Christopher Nolan",
    "coDirectorCount": 1
  },
  "Punch-Drunk Love": {
    "name": "Paul Thomas Anderson",
    "coDirectorCount": 1
  },
  "Best in Show": {
    "name": "Christopher Guest",
    "coDirectorCount": 1
  },
  "Uncut Gems": {
    "name": "Benny Safdie",
    "coDirectorCount": 2
  },
  "Toni Erdmann": {
    "name": "Maren Ade",
    "coDirectorCount": 1
  },
  "No Country for Old Men": {
    "name": "Ethan Coen",
    "coDirectorCount": 2
  },
  "Whiplash": {
    "name": "Damien Chazelle",
    "coDirectorCount": 1
  },
  "Kill Bill Vol. 1": {
    "name": "Quentin Tarantino",
    "coDirectorCount": 1
  },
  "Memento": {
    "name": "Christopher Nolan",
    "coDirectorCount": 1
  },
  "Little Miss Sunshine": {
    "name": "Valerie Faris",
    "coDirectorCount": 2
  },
  "Gone Girl": {
    "name": "David Fincher",
    "coDirectorCount": 1
  },
  "Oppenheimer": {
    "name": "Christopher Nolan",
    "coDirectorCount": 1
  },
  "Spotlight": {
    "name": "Tom McCarthy",
    "coDirectorCount": 1
  },
  "Tár": {
    "name": "Todd Field",
    "coDirectorCount": 1
  },
  "The Hurt Locker": {
    "name": "Kathryn Bigelow",
    "coDirectorCount": 1
  },
  "Under the Skin": {
    "name": "Jonathan Glazer",
    "coDirectorCount": 1
  },
  "Eternal Sunshine of the Spotless Mind": {
    "name": "Michel Gondry",
    "coDirectorCount": 1
  },
  "Let the Right One In": {
    "name": "Tomas Alfredson",
    "coDirectorCount": 1
  },
  "Ocean's Eleven": {
    "name": "Steven Soderbergh",
    "coDirectorCount": 1
  },
  "Carol": {
    "name": "Todd Haynes",
    "coDirectorCount": 1
  },
  "Ratatouille": {
    "name": "Brad Bird",
    "coDirectorCount": 2
  },
  "The Florida Project": {
    "name": "Sean Baker",
    "coDirectorCount": 1
  },
  "Amour": {
    "name": "Michael Haneke",
    "coDirectorCount": 1
  },
  "O Brother, Where Art Thou?": {
    "name": "Joel Coen",
    "coDirectorCount": 2
  },
  "Everything Everywhere All at Once": {
    "name": "Dan Kwan",
    "coDirectorCount": 2
  },
  "Aftersun": {
    "name": "Charlotte Wells",
    "coDirectorCount": 1
  },
  "The Tree of Life": {
    "name": "Terrence Malick",
    "coDirectorCount": 1
  },
  "Get Out": {
    "name": "Jordan Peele",
    "coDirectorCount": 1
  },
  "Volver": {
    "name": "Pedro Almodóvar",
    "coDirectorCount": 1
  },
  "Black Swan": {
    "name": "Darren Aronofsky",
    "coDirectorCount": 1
  },
  "The Act of Killing": {
    "name": "Joshua Oppenheimer",
    "coDirectorCount": 2
  },
  "Inside Llewyn Davis": {
    "name": "Ethan Coen",
    "coDirectorCount": 2
  },
  "Melancholia": {
    "name": "Lars von Trier",
    "coDirectorCount": 1
  },
  "Anchorman": {
    "name": "Adam McKay",
    "coDirectorCount": 1
  },
  "Past Lives": {
    "name": "Celine Song",
    "coDirectorCount": 1
  },
  "The Lord of the Rings: The Fellowship of the Ring": {
    "name": "Peter Jackson",
    "coDirectorCount": 1
  },
  "The Gleaners & I": {
    "name": "Agnès Varda",
    "coDirectorCount": 1
  },
  "Interstellar": {
    "name": "Christopher Nolan",
    "coDirectorCount": 1
  },
  "Spirited Away": {
    "name": "Hayao Miyazaki",
    "coDirectorCount": 1
  },
  "Frances Ha": {
    "name": "Noah Baumbach",
    "coDirectorCount": 1
  },
  "Fish Tank": {
    "name": "Andrea Arnold",
    "coDirectorCount": 1
  },
  "Gladiator": {
    "name": "Ridley Scott",
    "coDirectorCount": 1
  },
  "Michael Clayton": {
    "name": "Tony Gilroy",
    "coDirectorCount": 1
  },
  "Minority Report": {
    "name": "Steven Spielberg",
    "coDirectorCount": 1
  },
  "The Worst Person in the World": {
    "name": "Joachim Trier",
    "coDirectorCount": 1
  },
  "Black Panther": {
    "name": "Ryan Coogler",
    "coDirectorCount": 1
  },
  "Gravity": {
    "name": "Alfonso Cuarón",
    "coDirectorCount": 1
  },
  "Grizzly Man": {
    "name": "Werner Herzog",
    "coDirectorCount": 1
  },
  "Memories of Murder": {
    "name": "Bong Joon-ho",
    "coDirectorCount": 1
  }
};

export const CURATED_SHOW_CREATOR = {
  "The Sopranos": "David Chase",
  "The Mary Tyler Moore Show": "James L. Brooks",
  "What We Do in the Shadows": "Jemaine Clement",
  "Succession": "Jesse Armstrong",
  "The Twilight Zone": "Rod Serling",
  "Veep": "Armando Iannucci",
  "The Americans": "Joe Weisberg",
  "The Larry Sanders Show": "Garry Shandling",
  "Twin Peaks": "Mark Frost",
  "The Leftovers": "Tom Perrotta",
  "Saturday Night Live": "Lorne Michaels",
  "I May Destroy You": "Michaela Coel",
  "The Simpsons": "Matt Groening",
  "30 Rock": "Tina Fey",
  "All in the Family": "Norman Lear",
  "Star Trek": "Gene Roddenberry",
  "Watchmen": "Damon Lindelof",
  "Freaks and Geeks": "Paul Feig",
  "MAS*H": "Larry Gelbart",
  "Sesame Street": "Joan Ganz Cooney",
  "Deadwood": "David Milch",
  "Friday Night Lights": "Peter Berg",
  "Breaking Bad": "Vince Gilligan",
  "Parks and Recreation": "Greg Daniels",
  "Game of Thrones": "D. B. Weiss",
  "Better Call Saul": "Vince Gilligan",
  "Monty Python's Flying Circus": "Monty Python",
  "The Office": "Greg Daniels",
  "Lost": "Damon Lindelof",
  "I Love Lucy": "Desi Arnaz",
  "Arrested Development": "Mitchell Hurwitz",
  "Hill Street Blues": "Steven Bochco",
  "Curb Your Enthusiasm": "Larry David",
  "The Wire": "David Simon",
  "The Good Place": "Michael Schur",
  "BoJack Horseman": "Raphael Bob-Waksberg",
  "Battlestar Galactica": "Ronald D. Moore",
  "Insecure": "Issa Rae",
  "Late Night With David Letterman": "David Letterman",
  "Columbo": "Richard Levinson",
  "The West Wing": "Aaron Sorkin",
  "My So-Called Life": "Winnie Holzman",
  "The Shield": "Shawn Ryan",
  "Friends": "David Crane",
  "Fleabag": "Phoebe Waller-Bridge",
  "Jeopardy!": "Merv Griffin",
  "The X-Files": "Chris Carter",
  "Barry": "Bill Hader",
  "The Office (UK)": "Ricky Gervais",
  "ER": "Michael Crichton",
  "Halt and Catch Fire": "Chris Cantwell",
  "Community": "Dan Harmon",
  "Russian Doll": "Natasha Lyonne",
  "Six Feet Under": "Alan Ball",
  "Key & Peele": "Keegan-Michael Key",
  "Seinfeld": "Larry David",
  "Taxi": "James L. Brooks",
  "The Underground Railroad": "Barry Jenkins",
  "The Dick Van Dyke Show": "Carl Reiner",
  "South Park": "Trey Parker",
  "The Golden Girls": "Susan Harris",
  "Girls": "Lena Dunham",
  "The Daily Show With Jon Stewart": "Madeleine Smithberg",
  "NYPD Blue": "Steven Bochco",
  "Fawlty Towers": "John Cleese",
  "Chappelle's Show": "Dave Chappelle",
  "Mad Men": "Matthew Weiner",
  "Better Things": "Louis C.K.",
  "Good Times": "Mike Evans",
  "Buffy the Vampire Slayer": "Joss Whedon",
  "The Honeymooners": "Jackie Gleason",
  "Frasier": "David Angell",
  "Justified": "Graham Yost",
  "The Jeffersons": "Don Nicholl",
  "Sex and the City": "Darren Star",
  "Mr. Show with Bob and David": "Bob Odenkirk",
  "Cheers": "James Burrows",
  "It's Always Sunny in Philadelphia": "Rob McElhenney",
  "Party Down": "Rob Thomas",
  "I'm Alan Partridge": "Peter Baynham",
  "Fargo": "Noah Hawley",
  "Orange Is the New Black": "Jenji Kohan",
  "The Bob Newhart Show": "David Davis",
  "The Crown": "Peter Morgan",
  "The Carol Burnett Show": "Carol Burnett",
  "Atlanta": "Donald Glover",
  "The Wonder Years": "Neal Marlens",
  "The Muppet Show": "Jim Henson",
  "The Rockford Files": "Roy Huggins",
  "NewsRadio": "Paul Simms",
  "Squid Game": "Hwang Dong-hyuk",
  "Rick and Morty": "Justin Roiland",
  "The Odd Couple": "Garry Marshall",
  "The Good Fight": "Michelle King",
  "Oz": "Tom Fontana"
};

export const CURATED_GAME_STUDIO = {
  "Counter-Strike 2": "Valve",
  "ARC Raiders": "Embark Studios",
  "Civilization VII": "semiwork",
  "Grand Theft Auto V Legacy": "Rockstar North",
  "War Thunder": "Gaijin Entertainment",
  "Overwatch 2": "Blizzard Entertainment",
  "Stardew Valley": "ConcernedApe",
  "Terraria": "Re-Logic",
  "Geometry Dash": "RobTop Games",
  "Grand Theft Auto V Enhanced": "Rockstar North",
  "Mewgenics": "Edmund McMillen",
  "NARAKA: BLADEPOINT": "24 Entertainment",
  "Dota 2": "Valve",
  "HELLDIVERS 2": "Arrowhead Game Studios",
  "Monster Hunter Wilds": "CAPCOM",
  "Warframe": "Digital Extremes",
  "Tom Clancy's Rainbow Six Siege X": "Ubisoft Montreal",
  "Dead by Daylight": "Behaviour Interactive",
  "Limbus Company": "ProjectMoon",
  "tModLoader": "TML Team",
  "VRChat": "VRChat",
  "Team Fortress 2": "Valve",
  "Euro Truck Simulator 2": "SCS Software",
  "PUBG: BATTLEGROUNDS": "PUBG Corporation",
  "EA SPORTS FC 26": "EA Canada",
  "Marvel Rivals": "NetEase Games",
  "Hearts of Iron IV": "Paradox Development Studio",
  "Heartopia": "XD",
  "Baldur's Gate 3": "Larian Studios",
  "Battlefield 6": "Battlefield Studios",
  "Palworld": "Pocketpair",
  "ELDEN RING NIGHTREIGN": "FromSoftware",
  "DayZ": "Bohemia Interactive",
  "Crosshair X": "CenterPoint Gaming",
  "Apex Legends": "Respawn",
  "Where Winds Meet": "Everstone Studio",
  "Football Manager 26": "Sports Interactive",
  "Sid Meier's Civilization VI": "Firaxis Games",
  "Don't Starve Together": "Klei Entertainment",
  "R.E.P.O.": "semiwork",
  "Red Dead Redemption 2": "Rockstar Games",
  "RimWorld": "Ludeon Studios",
  "Cyberpunk 2077": "CD PROJEKT RED",
  "The Sims 4": "Maxis",
  "Project Zomboid": "The Indie Stone",
  "Rust": "Facepunch Studios",
  "ELDEN RING": "FromSoftware",
  "PEAK": "Team PEAK",
  "Call of Duty": "Treyarch",
  "ARK: Survival Ascended": "Studio Wildcard",
  "Street Fighter 6": "CAPCOM",
  "Warhammer 40K: Space Marine 2": "Saber Interactive",
  "Slay the Spire": "Mega Crit",
  "Garry's Mod": "Facepunch Studios",
  "NBA 2K26": "Visual Concepts",
  "Total War: WARHAMMER III": "CREATIVE ASSEMBLY",
  "Bongo Cat": "Marcel Zurawka",
  "Mount & Blade II: Bannerlord": "TaleWorlds Entertainment",
  "Kingdom Come: Deliverance II": "Warhorse Studios",
  "Shadowverse: Worlds Beyond": "Cygames",
  "Farming Simulator 25": "GIANTS Software",
  "Arena Breakout: Infinite": "Morefun Studios",
  "BeamNG.drive": "BeamNG",
  "The Elder Scrolls V: Skyrim Special Edition": "Bethesda Game Studios",
  "Hunt: Showdown 1896": "Crytek",
  "Left 4 Dead 2": "Valve",
  "The Binding of Isaac: Rebirth": "Nicalis",
  "Delta Force": "Team Jade",
  "It Takes Two": "Hazelight Studios",
  "Fallout 4": "Bethesda Game Studios",
  "Crusader Kings III": "Paradox Development Studio",
  "Age of Empires II: Definitive Edition": "World's Edge",
  "Valheim": "Iron Gate",
  "PAYDAY 2": "OVERKILL",
  "Black Myth: Wukong": "Game Science",
  "Core Keeper": "Pugstorm",
  "eFootball": "KONAMI",
  "Yu-Gi-Oh! Master Duel": "KONAMI",
  "Wallpaper Engine": "Wallpaper Engine Team",
  "Monster Hunter: World": "CAPCOM",
  "Satisfactory": "Coffee Stain Studios",
  "Forza Horizon 5": "Playground Games",
  "Resident Evil 4": "CAPCOM",
  "Factorio": "Wube Software",
  "Squad": "Offworld",
  "7 Days to Die": "The Fun Pimps",
  "Tabletop Simulator": "Berserk Games",
  "Black Desert": "Pearl Abyss",
  "ARK: Survival Evolved": "Studio Wildcard",
  "Oxygen Not Included": "Klei Entertainment",
  "Banana": "Sky",
  "Football Manager 2024": "Sports Interactive",
  "Counter-Strike: Source": "Valve",
  "Russian Fishing 4": "FishSoft",
  "Soundpad": "Leppsoft",
  "IdleOn - The Idle RPG": "Lavaflame2",
  "Eternal Return": "Nimble Neuron",
  "Deep Rock Galactic": "Ghost Ship Games",
  "Hades": "Supergiant Games"
};

// Curated Top 100 Book/Movie/Game/Show items store "Title — Creator" combined in a single
// .title field (with the real plot synopsis/description in .notes) — unlike Music Album, which
// gets a clean title plus the artist name separately in .notes. Confirmed the same combined-title
// convention applies to Book (Movie/Game/Show titles turned out to be plain — see
// getStaticCuratedCreator below for how those resolve creator names instead).
export const SPLIT_TITLE_CREATOR_CATEGORIES = ['Book', 'Movie', 'Game', 'Show'];
export function splitCuratedTitleCreator(title) {
  const idx = title?.indexOf(' — ');
  if (idx == null || idx === -1) return { title, author: null };
  return { title: title.slice(0, idx), author: title.slice(idx + 3) };
}

// Movie/Show/Game curated items have no creator embedded in Firestore at all (unlike Book's
// combined title) — resolved externally and kept as static in-app data above. `name` stays the
// clean director/creator/studio name (used for navigation/matching); `hasMore` flags a
// co-directed movie so renderers can append a "…" purely for display, without corrupting the
// name used to link back to that director's own page.
export function getStaticCuratedCreator(cat, title) {
  if (cat === 'Movie') {
    const entry = CURATED_MOVIE_DIRECTOR[title];
    return entry ? { name: entry.name, hasMore: entry.coDirectorCount > 1 } : null;
  }
  if (cat === 'Show') {
    const name = CURATED_SHOW_CREATOR[title];
    return name ? { name, hasMore: false } : null;
  }
  if (cat === 'Game') {
    const name = CURATED_GAME_STUDIO[title];
    return name ? { name, hasMore: false } : null;
  }
  return null;
}
