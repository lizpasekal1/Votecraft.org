...
- **`js/profile.js`** (new) — `renderProfilePage()`, dispatched from `render.js`'s `renderGrid()` on `state.view === 'profile'`. Reached via Dashboard's Profile widget and Settings-dropdown `#btn-profile` (both in `dashboard.js`/`main.js`) — **both currently skip the sign-in check** (demo mode; look for the "Demo mode" comments to restore `getCurrentUser() ? ... : openAuthModal()` gating later). The sidebar used to also have a Profile link; it was removed (redundant with the other two entry points). Layout: Account card alone at top, full width; Connections/Interests/Your Music Taste/Friends share a `.profile-widget-grid` sized identically to the Dashboard's `.dash-card`/2×2 grid (reusing that exact class, now 800px tall — outgrew the Dashboard's original 500px once Connections became multi-row), each scrolling internally if content overflows its cell. Connections is one card holding a row per platform (`buildLastfmRow()`/`buildSteamRow()`/`buildInstagramRow()`), not one card per platform, to keep the widget-grid at exactly 4 cells.
- **`js/api.js`** — `ensureLastfmRecentTracks(username)`/`isLastfmConfigured()` (public `user.getrecenttracks`) and `ensureSteamRecentGames(usernameOrId)`/`isSteamConfigured()` (public `IPlayerService/GetRecentlyPlayedGames`, plus `_resolveSteamId()` for vanity-URL→SteamID64 resolution via `ISteamUser/ResolveVanityURL`). Both API key constants (`LASTFM_API_KEY`/`STEAM_API_KEY`) ship blank, same pattern as the pre-existing unused `YOUTUBE_API_KEY`. `manifest.json` needed `api.steampowered.com` added to `host_permissions` (the pre-existing Steam permission only covered the storefront domain).
...
- **Reload/persistence**: `main.js`'s `init()` used to force `state.view = 'dashboard'` on every load regardless of what was saved — removed. Every navigation call site (Dashboard link, Profile entry points, the hover-menu's Home/Curated/My Saves/Sponsored options, category/subfolder clicks) now calls `persistViewState()`, so a reload genuinely restores the last-active page. `state.js`'s default `view` changed from `'Books'` to `'dashboard'` so a first-ever load (no saved state) still lands somewhere sensible.
- **New "Webpages" sidebar tab**: `state.view = 'Web Links'` — filters items with `category === 'Web Links'` (the pseudo-category `main.js`'s streaming-URL auto-detect assigns). Not a `CATEGORIES` member (would affect the Add wizard/`CATEGORY_PLATFORMS` if added there), so `render.js` builds a local `sidebarCategoryList = ['Web Links', ...CATEGORIES...]` for sidebar rendering only, with `cat === 'Web Links'` special-cased for its icon/label. Gets full folder support this way for free (same `.sidebar-category`/`.sidebar-subfolder` handlers). Excluded from curated-genre drilldown mode (`sidebarCategoryList` omits it when `isCuratedGenre`). Three default folders (Articles/Blogs/News) seeded in `storage.js`'s `loadAll()`, same block as the Music Album/Book defaults. `state.js`'s default `collapsed` Set and `render.js`'s accordion-reset logic both had to be updated to include `'Web Links'` alongside `CATEGORIES` — easy to miss since most `CATEGORIES`-based logic elsewhere doesn't need the same treatment.
...
- **Sidebar collapsed-rail**: hover/active state now targets just `.cat-icon` instead of the full (much wider) row; `.sidebar-genre-arrow` (the `>` chevron in curated genre-picker mode) is hidden while collapsed.
