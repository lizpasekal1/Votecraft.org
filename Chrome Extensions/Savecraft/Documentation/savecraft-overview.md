...
- **Real bug found and fixed: toolbar spacing** — the gap separating the note-formatting buttons from the Expand/Close buttons was sized via `margin-left: auto`, which looked fine with 3 formatting buttons but grew into an oversized, unbalanced gap once Image became a 4th. Replaced with a fixed margin.
