...
// Renamed from the old technical names (Web Links/Show/Musician/Music Album/Game/Movie/Book/
// Visual Art) to match the friendly names every visitor already saw via the old CAT_LABEL map —
// the two are now unified, so CATEGORIES *is* the display name. See CAT_LABEL below (now an
// identity map, kept for the many call sites that still do `CAT_LABEL[cat] || cat`), and
// storage.js's _CAT_NORMALIZE/CAT_MIGRATION/the gated savecraft_category_rename_migrated
// migration for how existing Firestore data (curated and personal) converts to these values.
export const CATEGORIES = ['Sources', 'Series', 'Music', 'Albums', 'Games', 'Films', 'Literature', 'Arts'];
...
export const SUMMARY_PLACEHOLDER_CATEGORIES = ['Literature', 'Series', 'Films', 'Games'];
...
export const CATEGORY_WHY_TEXT = {
  Music: 'Music shapes culture, identity, and resistance — the same forces that drive civic change.',
  Albums: 'Music shapes culture, identity, and resistance — the same forces that drive civic change.',
  Series: 'The stories we follow on screen shape our empathy, our politics, and how we see each other.',
  Games: 'Games build communities, test strategy, and spark collaboration — skills at the core of civic life.',
  Literature: 'Great books expand our understanding of the world and each other — the foundation of an engaged citizenry.',
};
// Categories whose curated (Top 100) items stash the creator's name in a clean `.notes` field —
// confirmed Albums (formerly "Music Album") is the only one. Literature/Films/Games/Series
// instead combine "Title — Creator" into `.title`, with a real description in `.notes` — see
// splitCuratedTitleCreator() in render.js, which parses those apart into a real item.author at
// load time instead.
export const CURATED_NOTES_CATEGORIES = ['Albums'];
...
// under — Music already works this way natively (it IS a real category); Book Author/Movie
// Director/Show Creator/Game Studio are curated-only buckets (their own names left as-is — only
// the real CATEGORIES they map to were renamed) that map back to the real Literature/Films/
// Series/Games category for navigateToAuthor().
...
export const CURATED_GENRE_LANDING_CONTENT = {
  'Top 100': {
    ...
    categoryLogos: {
      Music: { imageUrl: 'images/logos/pngkey.com-rolling-stones-tongue-png-3135824.png', alt: 'Rolling Stone', wrapClassName: 'rs-logo-wrap', imgClassName: 'rs-logo-img' },
      Series: { imageUrl: 'images/logos/pngkey.com-rolling-stones-tongue-png-3135824.png', alt: 'Rolling Stone', wrapClassName: 'rs-logo-wrap', imgClassName: 'rs-logo-img' },
      Literature: { imageUrl: 'images/logos/pngkey.com-rolling-stones-tongue-png-3135824.png', alt: 'Rolling Stone', wrapClassName: 'rs-logo-wrap', imgClassName: 'rs-logo-img' },
      Games: { imageUrl: 'images/logos/steam.png', alt: 'Steam', wrapClassName: 'steam-logo-wrap', imgClassName: 'steam-logo-img' },
      Films: { svg: true, alt: 'The New York Times', wrapClassName: 'nyt-logo-wrap' },
    },
    rows: [
      ...
      // in render.js.
      { category: 'Music', label: 'Top Musicians', titles: ['The Beatles', 'Aretha Franklin', 'Queen', 'Stevie Wonder', 'The Rolling Stones', 'Joni Mitchell', 'Muse', 'Johnny Cash'] },
      { category: 'Films', label: 'Top Films', titles: ['Everything Everywhere All at Once', 'Eternal Sunshine of the Spotless Mind', 'Arrival', 'Spirited Away', 'Citizen Kane', 'The Godfather', '2001: A Space Odyssey'] },
      { category: 'Literature', label: 'Top Literature' },
      { category: 'Games', label: 'Top Games' },
    ],
  },
  ...
  'RCV': {
    ...
    rows: [
      ...
      { category: 'Literature', label: 'Books on Voting Reform' },
...
// Was a rename map (old technical name -> friendly display name) before CATEGORIES itself was
// renamed to match; now an identity map, kept (not deleted) purely so the many existing call
// sites that do `CAT_LABEL[cat] || cat` for display keep working completely unchanged.
export const CAT_LABEL = {
  Sources: 'Sources', News: 'News', Literature: 'Literature', Games: 'Games', Films: 'Films',
...
export const CAT_EMOJI = { 'News': '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#5B5BEF"><path d="M160-120q-33 0-56.5-23.5T80-200v-640l67 67 66-67 67 67 67-67 66 67 67-67 67 67 66-67 67 67 67-67 66 67 67-67v640q0 33-23.5 56.5T800-120H160Zm0-80h280v-240H160v240Zm360 0h280v-80H520v80Zm0-160h280v-80H520v80ZM160-520h640v-120H160v120Z"/></svg>', Albums: '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#5B5BEF"><path d="M500-360q42 0 71-29t29-71v-220h120v-80H560v220q-13-10-28-15t-32-5q-42 0-71 29t-29 71q0 42 29 71t71 29ZM320-240q-33 0-56.5-23.5T240-320v-480q0-33 23.5-56.5T320-880h480q33 0 56.5 23.5T880-800v480q0 33-23.5 56.5T800-240H320Zm0-80h480v-480H320v480ZM160-80q-33 0-56.5-23.5T80-160v-560h80v560h560v80H160Zm160-720v480-480Z"/></svg>', Music: CAT_EMOJI_NOTE_ICON, Series: '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#5B5BEF"><path d="m460-380 280-180-280-180v360ZM320-240q-33 0-56.5-23.5T240-320v-480q0-33 23.5-56.5T320-880h480q33 0 56.5 23.5T880-800v480q0 33-23.5 56.5T800-240H320ZM160-80q-33 0-56.5-23.5T80-160v-560h80v560h560v80H160Z"/></svg>', Literature: '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#5B5BEF"><path d="M270-80q-45 0-77.5-30.5T160-186v-558q0-38 23.5-68t61.5-38l395-78v640l-379 76q-9 2-15 9.5t-6 16.5q0 11 9 18.5t21 7.5h450v-640h80v720H270Zm10-217 80-16v-478l-80 16v478Z"/></svg>', Films: '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 2 24 24" width="24px" fill="#5B5BEF"><path d="m7.727,7L13.727,1h3.422l-6,6h-3.422Zm9.672,0l5.4-5.4c-.501-.377-1.124-.6-1.798-.6h-1.023l-6,6h3.422Zm6.52-3.692l-3.692,3.692h3.773v-3c0-.238-.029-.47-.081-.692ZM4.898,7L10.898,1h-3.403L1.496,7h3.403ZM3,1C1.346,1,0,2.346,0,4v1.667L4.667,1h-1.667ZM0,9h24v14H0v-14Zm10,10.5l5.833-3.5-5.833-3.5v7Z"/></svg>', Games: '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#5B5BEF"><path d="M182-200q-51 0-79-35.5T82-322l42-300q9-60 53.5-99T282-760h396q60 0 104.5 39t53.5 99l42 300q7 51-21 86.5T778-200q-21 0-39-7.5T706-230l-90-90H344l-90 90q-15 15-33 22.5t-39 7.5Zm526.5-251.5Q720-463 720-480t-11.5-28.5Q697-520 680-520t-28.5 11.5Q640-497 640-480t11.5 28.5Q663-440 680-440t28.5-11.5Zm-80-120Q640-583 640-600t-11.5-28.5Q617-640 600-640t-28.5 11.5Q560-617 560-600t11.5 28.5Q583-560 600-560t28.5-11.5ZM310-440h60v-70h70v-60h-70v-70h-60v70h-70v60h70v70Z"/></svg>', Arts: '<svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0
300-480q0-21 1.5-40.5T306-560H170q-5 20-7.5 39.5T160-480q0 21 2.5 40.5T170-400Zm216 0h188q3-20 4.5-39.5T580-480q0-21-1.5-40.5T574-560H386q-3 20-4.5 39.5T380-480q0 21 1.5 40.5T386-400Zm268 0h136q5-20 7.5-39.5T800-480q0-21-2.5-40.5T790-560H654q3 20 4.5 39.5T660-480q0 21-1.5 40.5T654-400Zm-16-240h118q-29-50-72.5-87T584-782q18 33 31.5 68.5T638-640Zm-234 0h152q-12-44-31-83t-45-75q-26 36-45 75t-31 83Zm-200 0h118q9-38 22.5-73.5T376-782q-56 18-99.5 55T204-640Z"/></svg>' };
...
// PRIMARY_FOLDER_ID['Music'], the same destination the sidebar's own plain "Musicians" row
...
export const CATEGORY_PLATFORMS = {
  Albums:  { label: 'Sources', platforms: [
  ...
  Music:          { label: 'Sources', platforms: null },
  'Favorite Albums': { label: 'Sources', platforms: null },
  Series:         { label: 'Sources', platforms: [
  ...
  Films:          { label: 'Sources', platforms: null },
  Games:          { label: 'Sources', platforms: [
  ...
  Literature:     { label: 'Sources', platforms: [
...
CATEGORY_PLATFORMS['Music'].platforms              = CATEGORY_PLATFORMS['Albums'].platforms;
CATEGORY_PLATFORMS['Favorite Albums'].platforms    = CATEGORY_PLATFORMS['Albums'].platforms;
CATEGORY_PLATFORMS['Films'].platforms              = CATEGORY_PLATFORMS['Series'].platforms;
...
export const PRIMARY_FOLDER_ID = {
  Films: 'default-movies-movies',
  Music: 'default-musicians-musicians',
  Albums: 'default-music-albums',
  Literature: 'default-books-books',
  Sources: 'default-weblinks-websites',
};
...
export const FOLDER_ICON = {
  ...
  // 'default-shows-creators' icon retired along with that folder (storage.js) — replaced by
  // 'default-shows-shortform' below, per direct request.
  'default-shows-shortform':    { type: 'svg', path: 'M600-160q-134 0-227-93t-93-227q0-133 93-226.5T600-800q133 0 226.5 93.5T920-480q0 134-93.5 227T600-160Zm0-80q100 0 170-70t70-170q0-100-70-170t-170-70q-100 0-170 70t-70 170q0 100 70 170t170 70Zm91-91 57-57-108-108v-144h-80v177l131 132ZM80-600v-80h160v80H80ZM40-440v-80h200v80H40Zm40 160v-80h160v80H80Zm520-200Z' },
...
export const state = {
  ...
  itemWikiCache: {}, // { [normalizedTitle]: { bio: string|null, photoUrl: string|null, fetchedAt: number } } — auto-fetched via Wikipedia for Literature/Series/Films/Games items
  creatorCache: {}, // { [category:title]: { creator: string|null, fetchedAt: number } } — Films director/Series creator via Wikidata, Games studio via Steam
  tutorialSeen: false,
  ...
  adminKanbanCards: [], // { id, name, details, urgency, status: ADMIN_KANBAN_COLUMNS key,
                         // manualOrder, createdAt, pinned?, pinnedAt? } — a second, separate board
  ...
  curatedTopics: {}, // { [topicSlug]: { name, slug, shortName, headline, description, iconUrl,
                     ...
                     //   builds the aggregate view by pooling every curatedLists entry whose
                     //   `topics` includes this slug.
};
