...
function vc_sidebar_handle_save() {
    ...
    $settings['menu_spacing_mobile'] = intval( $_POST['vc_menu_spacing_mobile'] ?? 4 );

    // Crossfade
    $settings['crossfade'] = array(
        'enabled'  => ! empty( $_POST['vc_crossfade_enabled'] ),
        'fade_out' => max( 0, min( 2000, intval( $_POST['vc_crossfade_out'] ?? 400 ) ) ),
        'fade_in'  => max( 0, min( 2000, intval( $_POST['vc_crossfade_in'] ?? 450 ) ) ),
    );
...
function vc_sidebar_admin_page() {
    ...
    <div class="wrap">
        ...
        <form method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td><input type="number" name="vc_menu_spacing"
                               ...
                               min="0" max="40" step="1">
                        <p class="description">Vertical gap between menu items on desktop (default: 4px)</p></td>
                </tr>
                <tr>
                    <th>Mobile Menu Item Spacing (px)</th>
                    <td><input type="number" name="vc_menu_spacing_mobile"
                               value="<?php echo intval( $s['menu_spacing_mobile'] ?? 4 ); ?>"
                               min="0" max="40" step="1">
                        <p class="description">Vertical gap between menu items on mobile (default: 4px)</p></td>
                </tr>
                <tr>
                    <th>Mobile Style</th>
                    <td>
            ...
            <table class="form-table">
                <tr>
                    <th>Enable Crossfade</th>
                    <td>
                        <label>
                            <input type="checkbox" name="vc_crossfade_enabled" value="1"
                                   <?php checked( $s['crossfade']['enabled'] ?? true ); ?>>
                            Smooth crossfade when navigating between pages
                        </label>
                    </td>
                </tr>
                <tr>
                    <th>Fade Out Duration (ms)</th>
                    <td>
                        <input type="range" name="vc_crossfade_out" id="vc-fade-out-range"
                               value="<?php echo intval( $s['crossfade']['fade_out'] ?? 400 ); ?>"
                               min="100" max="1500" step="50">
                        <span id="vc-fade-out-val"><?php echo intval( $s['crossfade']['fade_out'] ?? 400 ); ?>ms</span>
                        <p class="description">How long the current page takes to fade out (100–1500ms)</p>
                    </td>
                </tr>
                <tr>
                    <th>Fade In Duration (ms)</th>
                    <td>
                        <input type="range" name="vc_crossfade_in" id="vc-fade-in-range"
                               value="<?php echo intval( $s['crossfade']['fade_in'] ?? 450 ); ?>"
                               min="100" max="1500" step="50">
                        <span id="vc-fade-in-val"><?php echo intval( $s['crossfade']['fade_in'] ?? 450 ); ?>ms</span>
                        <p class="description">How long the new page takes to fade in (100–1500ms)</p>
                    </td>
                </tr>
            </table>
    ...
    jQuery(function($) {
        ...
        $('#vc-fade-out-range').on('input', function() {
            $('#vc-fade-out-val').text(this.value + 'ms');
        });
        $('#vc-fade-in-range').on('input', function() {
            $('#vc-fade-in-val').text(this.value + 'ms');
        });
    });
    </script>
    <?php
...
function vc_sidebar_enqueue() {
    ...
    $custom_css = ':root {' .
        ...
        '--vc-fade-out: '               . intval( $s['crossfade']['fade_out'] ?? 400 )   . 'ms;' .
        '--vc-fade-in: '                . intval( $s['crossfade']['fade_in'] ?? 450 )    . 'ms;' .
    '}' .
    '@media (max-width: 768px) {:root {' .
        '--vc-sidebar-spacing: '        . intval( $s['menu_spacing_mobile'] ?? 4 )       . 'px;' .
    '}}';
...
function vc_sidebar_render() {
    ...
    <nav id="vc-sidebar" class="vc-sidebar" role="navigation" aria-label="Main navigation"
         data-crossfade="<?php echo ( $s['crossfade']['enabled'] ?? true ) ? '1' : '0'; ?>"
         data-fade-out="<?php echo intval( $s['crossfade']['fade_out'] ?? 400 ); ?>"
         data-fade-in="<?php echo intval( $s['crossfade']['fade_in'] ?? 450 ); ?>">
            ...
            <?php foreach ( $items as $item ) :
                ...
                if ( ! $is_active && $current_page_ancestors ) {
                    ...
                    if ( $menu_page_id && in_array( $menu_page_id, $current_page_ancestors, true ) ) {
                        ...
                        // Find direct child of menu page in ancestor chain
                        // get_post_ancestors returns [parent, grandparent, ...] (closest first)
                        $menu_idx = array_search( $menu_page_id, $current_page_ancestors );
                        if ( $menu_idx > 0 ) {
                            // Grandchild+ page: show intermediate parent as link
                            $direct_child_id = $current_page_ancestors[ $menu_idx - 1 ];
                            $subpage_title = get_the_title( $direct_child_id );
                            $subpage_url   = get_permalink( $direct_child_id );
                        } else {
                            // Direct child of menu page
                            $subpage_title = $current_page_title;
                            $subpage_url   = get_permalink( $current_page_obj->ID );
                        }
                    }
                }
                ?>
                <li class="vc-sidebar__item<?php echo $is_active ? ' vc-sidebar__item--active' : ''; ?>"
                    ...
                    <?php if ( $subpage_title ) : ?>
                        <a href="<?php echo esc_url( $subpage_url ); ?>" class="vc-sidebar__sublabel"><?php echo esc_html( $subpage_title ); ?></a>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
